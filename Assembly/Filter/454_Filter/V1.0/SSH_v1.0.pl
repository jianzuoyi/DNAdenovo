...
# change file name
die print LOG "$!\n" if system( "tar -zcf $dir/$out_pre.fa.tar.gz $dir/filter.*.fa " );
die print LOG "$!\n" if system( "tar -zcf $dir/$out_pre.qual.tar.gz $dir/filter.*.qual " );
# Clean the data
if ($clean){
	...
	die print LOG "$!\n" if system( "mv $dir/*.fa $dir/*.len $dir/*.qual $dir/*.id $dir/origin_data" );
    }
}
close LOG;
