...
if ( $type =~ /sff/i ) {
    ...
    if ( $out_pre =~ /\.gz$/ ) {
        open SEQ, "gunzip -dc $seq_file |"
          || die Message("Failed to open $seq_file");
    }
    else {
        open SEQ, $seq_file
          || die Message("Failed to open $seq_file");
...
open AINTACT, ">$out_dir/$out_pre.A.intact.deCS" || die Message("Failed to open filehandle AINTACT");
open ACS1, ">$out_dir/$out_pre.A.fragment.deCS1" || die Message("Failed to open filehandle ACS1");
open ACS2, ">$out_dir/$out_pre.A.fragment.deRcCS2" || die Message("Failed to open filehandle ACS2");

## B terminal output files: BINTACT & BCS1 & BCS2
open BINTACT, ">$out_dir/$out_pre.B.intact.deCS" || die Message("Failed to open filehandle BINTACT");
open BCS2, ">$out_dir/$out_pre.B.fragment.deCS2" || die Message("Failed to open filehandle BCS2");
open BCS1, ">$out_dir/$out_pre.B.fragment.deRcCS1" || die Message("Failed to open filehandle BCS1");

open NON, ">$out_dir/$out_pre.NON.fragment.noCS" || die Message("Failed to open filehandle NON");

## Cannot distiguish A or B terminal: NON
unless ( $remove =~ /y/i ) {
    open CSONLY, ">$out_dir/$out_pre.CsOnly" || die Message("Failed to open filehandle CSONLY");
    open CSWRONG, ">$out_dir/$out_pre.CsWrong" || die Message("Failed to open filehandle CSWRONG");
}

## GFF file for generating qualtity files for fasta format sequence file: GFF
open GFF, ">$out_dir/$out_pre.gff" || die Message("Failed to open filehandle GFF");
...
##====================##
	...
	if ( exists $trimCS->{non} ) {
            print NON ">$id\n$seq\n";
            print GFF "$id\t$full_length\t1\t$full_length\t-\t-\t-\t-\n";
        }
	## print A
	        ...
	        next if (!defined $trimCS->{A}->{$file_type}->{new_seq} || $trimCS->{A}->{$file_type}->{new_seq} eq '');
                print GFF "$id\t$full_length\t$trimCS->{A}->{$file_type}->{start}\t$trimCS->{A}->{$file_type}->{end}\t";
                print GFF "$trimCS->{A}->{$file_type}->{cs1}\t$trimCS->{A}->{$file_type}->{rc_cs2}\n";
	...
	## print B
	        ...
	        next if (!defined $trimCS->{B}->{$file_type}->{new_seq} || $trimCS->{B}->{$file_type}->{new_seq} eq '');
                print GFF "$id\t$full_length\t$trimCS->{B}->{$file_type}->{start}\t$trimCS->{B}->{$file_type}->{end}\t";
                print GFF "$trimCS->{B}->{$file_type}->{cs2}\t$trimCS->{B}->{$file_type}->{rc_cs1}\n";
