...
ForkManager - A simple parallel processing fork manager

=head1 SYNOPSIS

  use ForkManager;

  $pm = new ForkManager($MAX_PROCESSES);
...
you must instantiate another ForkManager object!
...
Instantiate a new ForkManager object. You must specify the maximum
...
Example of a program using callbacks to get child exit codes:
  ...
  my $pm = new ForkManager($max_procs);
...
In this simple example, each child sends back a string reference.
  ...
  use ForkManager 0.7.6;
  use strict;
  
  my $pm = new ForkManager(2, '/server/path/to/temp/dir/');
...
=for example begin
  ...
  my $pm = new ForkManager(20);  # using the system temp dir $L<File::Spec>->tmpdir()
...
Do not use ForkManager in an environment, where other child
processes can affect the run of the main program, so using this module
is not recommended in an environment where fork() / wait() is already used.

If you want to use more than one copies of the ForkManager, then
you have to make sure that all children processes are terminated, before you
use the second object in the main program.

You are free to use a new copy of ForkManager in the child
...
sub new { my ($c,$processes, $tempdir)=@_;
  ...
  $h->{tempdir} = $tempdir;  # add tempdir to ForkManager object so children can use it
