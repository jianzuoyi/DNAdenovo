...
void in_read1(string &seq,int n, Read1 &base)	//lzy 0924
{
//	Read1 base;
//<<lzy 0924
	for (int i=0; i<array_size; ++i)
...
int main (int argc, char *argv[])
{
	...
	igzstream infile1_tem( in_seq1.c_str());
	if ( ! infile1_tem)
	{
		cerr<<"fail to open input file"<<in_seq1<<endl;
	}
	getline ( infile1_tem, id1, '\n');
	getline ( infile1_tem, seq1, '\n');
	read1_len=seq1.length();

	igzstream infile2_tem( in_seq2.c_str());
	if ( ! infile2_tem)
	{
		cerr<<"fail to open input file"<<in_seq2<<endl;
	}
	getline ( infile2_tem, id2, '\n');
	getline ( infile2_tem, seq2, '\n');
	read2_len=seq2.length();

	total_len=read1_len+read2_len;
//	if (total_len>224)
	if (total_len>320)	//lzy 0924
	{
//		cerr<<"max pair reads length is 224"<<endl;
		cerr<<"max pair reads length is 320"<<endl;
		exit(1);
	}
	if (total_len<1)
	{
	...
	ogzstream outfile1 (out_seq1.c_str());
	if (! outfile1)
	{
		cerr << "fail to create output file" <<out_seq1<< endl;
	}
	ogzstream outfile2 (out_seq2.c_str());
	if (! outfile2)
	{
...
//lzy 0924			Read1 read_seq=in_read1(read,size_num);
			Read1 read_seq;
			in_read1(read,size_num, read_seq);
