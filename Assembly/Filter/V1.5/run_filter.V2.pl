...
while(<IN>){
	...
	print OUT1 "$Bin/filter_data_parallel -q 64 -t $thread_num -m $buffer_size -y -z -w $N_num -B $B_cutoff -l $Lib{$lib} -a $start1 -b $end1 -c $start2 -d $end2 $f1 $f2 ./$lib/$name.reads.stat ./$lib/$name.clean.tmp  ./$lib/$name2.clean.tmp && mv ./$lib/$name.clean.tmp ./$lib/$name.clean && mv ./$lib/$name2.clean.tmp ./$lib/$name2.clean && echo OK \n" ;
...
#my $qsge = "/home/lixiangfeng/Bin/qsub-sge.pl";
my $qsge = "/its1/GB_BT2/jianzuoyi/pipeline/DNA_Denovo/Filter/V1.5/qsub-sge.pl";
print STDERR "Runing low quality filtering ... \n";
`perl $qsge -maxjob $Maxjob -resource vf=3.5G -reqsub $file_name.filter.sh`;
print STDERR "Finished. \n";
print STDERR "Runing duplicate filtering ... \n";
`perl $qsge -maxjob $Maxjob -resource vf=$vf -reqsub $file_name.dup.sh`;
print STDERR "Finished. \n";
