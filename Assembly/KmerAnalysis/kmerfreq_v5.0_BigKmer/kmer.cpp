...
void Read2Kmer(int buffId, uint64_t readId, int threadId)
{
	...
	if(times_analysis > 1)
	{
		int right_move_bit = Kmer * 2 - prefix_bit_num;
  	for (string::size_type pos=0; pos<readBuffer[buffId][readId].size()-Kmer+1; pos++)
  	{
  		...
  		uint64_t rcKmerBit = get_rev_seq(kmerBit, Kmer);
  
  		if (kmerBit > rcKmerBit){
  			kmerBit = rcKmerBit;
  		}
  		if(((kmerBit >> right_move_bit) % times_analysis) != KPS){index++;continue;}
  		uint64_t hashcode = HashCode(kmerBit);
  		hcBuffer[buffId][index] = hashcode;
  		kmerStat[buffId][index] = 1;
...
void outputKmer(HashSet** &kmerHashSet, int threadNum, uint64_t*& freqy)
{
	...
	if (!fp){
		cerr << "Can't open kmerDepth.out.gz!\n";
	}
	for (int i=0; i<threadNum; i++){
		kmerNum += print_hashset(kmerHashSet[i], fp, freqy);
	}

	fp.close();

	cerr << "kmer number: " << kmerNum << endl;

	fp.open("kmerFreq.out");
	if (!fp){
		cerr << "Can't open kmerFreq.out!\n";
	}
...
int GetDepth(uint64_t* &freqy, int &min_depth, int &min_freq)
{
	...
	cout<<"Kmer_Frequence\tKmer_Species_Number\tKmer_ratio\tKmer_Individual_Number\n";
