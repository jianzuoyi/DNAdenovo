...
string Get_indel(string &seq){
	int seq_len=seq.size();
	int indel=int(seq_len*heterindel_rate);
	int array[3]={2,3,6};
	int p=1;
	for (int i=0;i<3;i++)
	{
		for (int j=0;j<indel/2/array[i];j++)
		{
...
void output_reads(vector <int> insert_dist,vector <double> err_dist,string &seq,int seqlen,
	int rd_pair,string id_seq,ofstream &o1,ofstream &o2,ofstream &log3){
	int reads_count=0;
	int k=0;
	...
	{	
		...
		{  
			...
			if (num1==0)
			{
				o1<<">"<<id_seq<<"_"<<insertsize_mean<<"_"<<read_length<<"_"<<reads_count<<"_"<<1<<endl<<read1<<endl;
				o2<<">"<<id_seq<<"_"<<insertsize_mean<<"_"<<read_length<<"_"<<reads_count<<"_"<<2<<endl<<read2<<endl;
			}else{
				o1<<">"<<id_seq<<"_"<<insertsize_mean<<"_"<<read_length<<"_"<<reads_count<<"_"<<1<<endl<<read2<<endl;
				o2<<">"<<id_seq<<"_"<<insertsize_mean<<"_"<<read_length<<"_"<<reads_count<<"_"<<2<<endl<<read1<<endl;
			}
			insert_dist[k]--;
		}
		k++;
...
void get_reads(string id,string &sequ,ofstream &of1,ofstream &of2,ofstream &log2){
	string sequence;
	...
	if (error_rate>0)
	{
		if (error_rate<0.002)
		{
			cout<<"Error:error_rate is smaller than basic error rate 0.002,you'd better set the value" 
...
void Get_genome(ifstream &inf,ofstream &outf1,ofstream &outf2,ofstream &log1){
	string line,id,seq;
	while (getline(inf,line,'\n'))
	{
		if (line[0]=='>')
		{
			...
			{	
				...
				get_reads(id,seq,outf1,outf2,log1);
				seq="";
			}
			line.erase(0,1);
			id=line;
...
int main(int argc, char *argv[])
{
	...
	if (!output)
	{
		char out1[]="solexa_1.fa";
		char out2[]="solexa_2.fa";
		char out3[]="solexa.log";
		outfile1.open(out1);
		outfile2.open(out2);
		log.open(out3);
	}else{
	...
	Get_genome(infile,outfile1,outfile2,log);
	return 0;
}
